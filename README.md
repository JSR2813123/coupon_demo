...
- 實驗目標：
理解 Tomcat Thread Pool 與 Hikari DB Connection Pool 的差異，並使用 Spring Boot Actuator 監控系統運行狀態。
Tomcat Thread Pool 負責處理 HTTP Request，DB Connection Pool 負責提供資料庫連線。兩者屬於不同層級的資源，即使 Tomcat 擁有大量 Thread，也可能因為 DB Connection 不足而產生等待。
透過 Actuator 監控：hikaricp.connections.active、hikaricp.connections.pending
觀察目前正在使用的 DB Connection 數量，以及等待 DB Connection 的 Request 數量。

- 實驗結論：
Actuator 可以即時觀察系統運行狀況，active 代表目前被使用中的 DB Connection，pending 代表等待取得 DB Connection 的 Request。
Tomcat Thread Pool 與 DB Connection Pool 並非相同資源，系統實際吞吐量可能受到較小的資源池限制，因此需要同時監控 Application 與 Database 層的資源使用情況。


## Tomcat thread pool vs DB connection thread實驗
- 實驗目標：
驗證 Tomcat Thread Pool 與 Hikari DB Connection Pool 對系統吞吐量的影響，觀察在高併發請求下，系統瓶頸是否來自 Application Thread 或 Database Connection。


- 實驗結論：
當 Hikari Pool 從 10 降至 3 時，active connection 接近上限，pending request 明顯增加，顯示 DB Connection Pool 已成為系統瓶頸。
當 Tomcat Thread Pool 從 100 提升至 300 時，pending request 從約 89 增加至約 289，但 Throughput 並未明顯提升。
實驗結果顯示，單純增加 Application Thread 無法突破資料庫層限制，系統瓶頸最終仍受到 DB Connection Pool 大小影響。



## index實驗
### 實驗目標：  
比較 `user_coupon.user_id` 在有無 Index 情況下的查詢效能差異，並觀察 PostgreSQL 的 Query Plan 變化。

### 實驗結論：  

10萬筆：Seq Scan → Bitmap Index Scan，Execution Time 從 7.580 ms 降至 0.146 ms

50萬筆：Parallel Seq Scan → Bitmap Index Scan，Execution Time 從 77.850 ms 降至 0.098 ms。

結果顯示，建立 Index 後 PostgreSQL 會選擇更有效率的 Query Plan，顯著降低查詢成本，並避免查詢時間隨資料量增加而線性成長。


## Docker 化部署與壓測

本專案已使用 Docker Compose 建立可重現的測試環境，包含：
...
在 Docker 環境中，Spring Boot 透過 Docker Compose service name 連線：

```yaml
PostgreSQL: jdbc:postgresql://postgres:5432/coupon_demo
Redis: redis:6379
```
...
```bash
docker exec -it coupon-redis redis-cli SET coupon_stock:1 10000
```
...
```text
POST http://localhost:8081/api/v1/coupon/1/claim/pessimistic
```
...
| Max        |       2744 ms |
| Error Rate |         0.00% |
| Throughput | 約 184.8 req/s |

結果顯示，在 Docker 環境下，系統可完成 2000 筆高併發請求，錯誤率為 0.00%。Redis 作為庫存 gate，負責高併發下的即時庫存扣減；PostgreSQL 則作為最終資料儲存，保存發券結果與一致性資料。

詳細 Docker 化與壓測紀錄請見：

```text
docs/高併發發券系統 Docker 化與壓測紀錄.docx
```
